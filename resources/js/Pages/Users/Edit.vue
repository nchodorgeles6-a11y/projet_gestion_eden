...
const departementId = ref(props.currentDepartementId ?? '');
...
const form = useForm({
    ...
    type_contrat:      props.user.type_contrat ?? 'employe',
    poste_id:          props.currentPosteId ?? '',
    salaire_base:      props.user.salaire_base ?? '',
    mode_paiement:     props.user.mode_paiement ?? 'fixe',
    prime_transport:   props.user.prime_transport ?? '',
    prime_logement:    props.user.prime_logement ?? '',
    prime_fonction:    props.user.prime_fonction ?? '',
    prime_rendement:   props.user.prime_rendement ?? '',
    prime_panier:      props.user.prime_panier ?? '',
    bonus_annuel:      props.user.bonus_annuel ?? '',
    cnps:              props.user.cnps ?? true,
    assurance_maladie: props.user.assurance_maladie ?? false,
    avantages_nature:  props.user.avantages_nature ?? [],
    date_fin_contrat:  props.user.date_fin_contrat ?? '',
});
...
<template>
    <Head title="Modifier le collaborateur" />
    <AuthenticatedLayout>
        <div class="p-6">
            ...
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h2 class="text-base font-extrabold" :class="dark ? 'text-white' : 'text-slate-800'">Modifier le collaborateur</h2>
                    <p class="text-[11px] mt-0.5" :class="sL(dark)">
                        {{ user.nom }} {{ user.prenom }} — Étape {{ currentStep }} sur {{ totalSteps }} — {{ steps[currentStep - 1].sub }}
                    </p>
                </div>
                <Link href="/users" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold border transition-all"
            ...
            <div class="rounded-2xl border transition-colors" :class="dark ? 'bg-[#161b22] border-[#21262d]' : 'bg-white border-slate-200 shadow-sm'">
                <form @submit.prevent="submit">
                    ...
                    <div v-show="currentStep === 4" class="p-4 sm:p-6 space-y-6">
                        <div>
