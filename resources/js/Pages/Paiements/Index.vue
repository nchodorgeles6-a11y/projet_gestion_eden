...
defineProps({ paiements: Object });

const destroy = (id) => {
    if (confirm('Voulez-vous supprimer ce paiement ?')) router.delete(`/paiements/${id}`);
};
</script>
    ...
    <AuthenticatedLayout>
        <div class="p-4 sm:p-6">
            ...
            <div class="flex items-center justify-between mb-5">
                <div>
                    <h2 class="text-base font-extrabold" :class="dark ? 'text-white' : 'text-slate-800'">Paiements</h2>
                    <p class="text-[11px] mt-0.5" :class="dark ? 'text-slate-400' : 'text-slate-500'">
                        {{ paiements?.meta?.total ?? 0 }} enregistrement(s)
                    </p>
                </div>
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr :class="dark ? 'bg-[#0d1117]' : 'bg-slate-50'">
                                ...
                                <th class="px-4 py-2.5 text-left text-[10px] font-bold tracking-wider uppercase" :class="dark ? 'text-slate-500' : 'text-slate-400'">Montant</th>
                                <th class="px-4 py-2.5 text-left text-[10px] font-bold tracking-wider uppercase" :class="dark ? 'text-slate-500' : 'text-slate-400'">Mois</th>
                                <th class="px-4 py-2.5 text-left text-[10px] font-bold tracking-wider uppercase" :class="dark ? 'text-slate-500' : 'text-slate-400'">Année</th>
                                <th class="px-4 py-2.5 text-left text-[10px] font-bold tracking-wider uppercase" :class="dark ? 'text-slate-500' : 'text-slate-400'">Référence</th>
                        ...
                        <tbody>
                            <tr v-for="(paiement, i) in paiements.data" :key="paiement.id"
                                ...
                                <td class="px-4 py-2.5 text-[12px]" :class="dark ? 'text-slate-300' : 'text-slate-600'">
                                    {{ paiement.user?.affectations?.[0]?.poste?.nom }}
                                </td>
                                <td class="px-4 py-2.5 text-[12px]" :class="dark ? 'text-slate-400' : 'text-slate-500'">
                                    {{ paiement.user?.affectations?.[0]?.poste?.departement?.nom }}
                                </td>
                                <td class="px-4 py-2.5 text-[12px] font-semibold" :class="dark ? 'text-emerald-400' : 'text-emerald-600'">
                                    {{ Number(paiement.montant).toLocaleString('fr-FR') }} FCFA
                                </td>
                                <td class="px-4 py-2.5 text-[12px]" :class="dark ? 'text-slate-400' : 'text-slate-500'">{{ paiement.mois }}</td>
                                <td class="px-4 py-2.5 text-[12px]" :class="dark ? 'text-slate-400' : 'text-slate-500'">{{ paiement.annee }}</td>
                                <td class="px-4 py-2.5 text-[12px] font-mono" :class="dark ? 'text-slate-400' : 'text-slate-500'">{{ paiement.reference }}</td>
                                <td class="px-4 py-2.5 text-right">
                                    <div class="inline-flex items-center gap-2">
                                        ...
                                        <button @click="destroy(paiement.id)"
                            ...
                            <tr v-if="!paiements?.data?.length">
                                <td colspan="8" class="px-4 py-8 text-center text-sm" :class="dark ? 'text-slate-500' : 'text-slate-400'">Aucun enregistrement.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                <Pagination :links="paiements.links" :meta="paiements" />
            </div>
        </div>
    </AuthenticatedLayout>
</template>
