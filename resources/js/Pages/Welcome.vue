...
<template>
    ...
    <Head title="Gestion RH | EDEN Corpor@te" />

    <div class="h-screen flex flex-col lg:flex-row font-sans overflow-hidden">

        <!-- COLONNE GAUCHE — Hero gradient -->
        <div class="relative flex-1 flex flex-col justify-between px-10 py-8 lg:px-14 lg:py-10 overflow-hidden
                    bg-gradient-to-br from-[#760078] via-[#4a0068] to-[#7677B7]">

            <!-- Formes décoratives -->
            <div class="absolute inset-0 pointer-events-none select-none">
                <div
                    class="absolute -bottom-16 -right-16 w-72 h-72 rounded-3xl bg-white/5 border border-white/10 rotate-12">
                </div>
                <div
                    class="absolute top-1/3 -right-10 w-48 h-48 rounded-2xl bg-white/5 border border-white/10 -rotate-6">
                </div>
                <div class="absolute top-20 -left-8 w-32 h-32 rounded-2xl bg-white/5 border border-white/10 rotate-45">
                </div>
                <div class="absolute top-1/2 left-1/3 w-56 h-56 rounded-full bg-[#760078]/30 blur-3xl -translate-y-1/2">
                </div>
                <div class="absolute bottom-10 right-1/4 w-40 h-40 rounded-full bg-[#7677B7]/40 blur-2xl"></div>
                <div class="absolute top-40 right-24 w-10 h-10 bg-white/10 rounded-md rotate-45"></div>
                <div class="absolute bottom-32 left-16 w-7 h-7 bg-white/10 rounded-md rotate-12"></div>
                <div class="absolute top-60 left-1/2 w-5 h-5 bg-white/15 rounded-sm -rotate-12"></div>
            </div>
            ...
            <div class="relative z-10 flex items-center gap-3">
                <div class="relative w-12 h-10 flex items-center justify-center">
                    <div
                        class="absolute w-6 h-6 border-[3px] border-white/80 rounded-md transform rotate-12 -translate-x-2">
                    </div>
                    <div
                        class="absolute w-5 h-5 border-[3px] border-white/60 rounded-md transform -rotate-6 translate-x-3 -translate-y-2">
                    </div>
                </div>
                <span class="text-2xl font-extrabold tracking-tight text-white select-none">
                    Eden<span class="font-light opacity-90">Corpor@te</span>
                </span>
            </div>
            ...
            <div class="relative z-10 text-white/40 text-xs">
                © {{ new Date().getFullYear() }} Eden Corpor@te · Tous droits réservés
            </div>
        </div>

        <!-- COLONNE DROITE — Formulaire de connexion -->
        <div class="w-full lg:w-1/2 xl:w-1/2 flex flex-col items-center justify-center
                    bg-[#f8f9fa] px-8 lg:px-14 overflow-y-auto">
            ...
            <div class="w-full max-w-sm py-8">
                ...
                <div v-if="status" class="mb-5 p-4 rounded-xl bg-emerald-50 border border-emerald-100
                    ...
                    <form @submit.prevent="submit" class="space-y-5">
                        ...
                        <div class="space-y-1.5">
                            ...
                            <div class="relative">
                                ...
                                <input id="email" type="email" v-model="form.email" required autofocus
                                    placeholder="nom.prenom@edencorporate.com" autocomplete="username" class="block w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200
                        ...
                        <div class="space-y-1.5">
                            <div class="flex items-center justify-between">
                                ...
                                <Link v-if="canResetPassword" :href="route('password.request')" class="text-xs font-semibold text-[#760078] hover:text-[#5a005c]
                            ...
                            <div class="relative">
                                ...
                                <input id="password" type="password" v-model="form.password" required
                                    placeholder="••••••••" autocomplete="current-password" class="block w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200
                                           rounded-xl text-slate-900 placeholder-slate-400 text-sm
                                           focus:outline-none focus:border-[#760078] focus:ring-2
                                           focus:ring-[#760078]/20 transition-all" />
                            </div>
                        ...
                        <div class="flex items-center gap-2 pt-1">
                            <input id="remember" type="checkbox" v-model="form.remember" class="w-4 h-4 rounded border-slate-300 text-[#760078]
                        ...
                        <div class="pt-2">
                            <button type="submit" :disabled="form.processing"
                                :class="{ 'opacity-70 cursor-wait': form.processing }" class="w-full flex items-center justify-center gap-2 py-3 px-4 text-sm font-semibold
                                ...
                                <svg v-if="form.processing" class="animate-spin h-4 w-4 text-white" fill="none"
                                    viewBox="0 0 24 24">
