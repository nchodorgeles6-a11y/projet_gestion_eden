...
const { dark } = useTheme();
const props = defineProps({ paiement: Object, users: Array });
const form = useForm({
    user_id: props.paiement.user_id,
    montant: props.paiement.montant,
    mois: props.paiement.mois,
    annee: props.paiement.annee,
});
const submit = () => form.put(`/paiements/${props.paiement.id}`);
...
const annees = ['2023','2024','2025','2026','2027','2028','2029'];
</script>

<template>
    <Head title="Modifier Paiement" />
    <AuthenticatedLayout>
        <div class="p-4 sm:p-6 max-w-2xl">
            <div class="flex items-center justify-between mb-5">
                <div>
                    <h2 class="text-base font-extrabold" :class="dark ? 'text-white' : 'text-slate-800'">Modifier le paiement</h2>
                    <p class="text-[11px] mt-0.5" :class="dark ? 'text-slate-400' : 'text-slate-500'">Modifiez les informations ci-dessous</p>
                </div>
                <Link href="/paiements" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold border transition-all"
            ...
            <div class="rounded-2xl border p-6 transition-colors"
                ...
                <form @submit.prevent="submit" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input v-model="form.montant" type="number" required placeholder="Ex: 250000"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select v-model="form.mois" required class="w-full rounded-xl border px-3 py-2.5 text-sm transition-all focus:outline-none focus:ring-2" :class="inputCls(dark)">
                        ...
                        <div>
                            ...
                            <select v-model="form.annee" required class="w-full rounded-xl border px-3 py-2.5 text-sm transition-all focus:outline-none focus:ring-2" :class="inputCls(dark)">
