...
defineProps({ departements: Object });

const destroy = (id) => {
    if (confirm('Voulez-vous supprimer ce département ?')) router.delete(`/departements/${id}`);
};
</script>
    ...
    <AuthenticatedLayout>
        <div class="p-4 sm:p-6">
            ...
            <div class="flex items-center justify-between mb-5">
                <div>
                    <h2 class="text-base font-extrabold" :class="dark ? 'text-white' : 'text-slate-800'">Départements</h2>
                    <p class="text-[11px] mt-0.5" :class="dark ? 'text-slate-400' : 'text-slate-500'">
                        {{ departements?.meta?.total ?? 0 }} enregistrement(s)
                    </p>
                </div>
            ...
            <div class="rounded-2xl border overflow-hidden transition-colors"
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody>
                        <tr v-for="(departement, i) in departements.data" :key="departement.id"
                            ...
                            <td class="px-4 py-2.5 text-[12px] font-medium" :class="dark ? 'text-slate-200' : 'text-slate-700'">{{ departement.nom }}</td>
                            <td class="px-4 py-2.5 text-right">
                                <div class="inline-flex items-center gap-2">
                                    ...
                                    <button @click="destroy(departement.id)"
                        ...
                        <tr v-if="!departements?.data?.length">
                            <td colspan="2" class="px-4 py-8 text-center text-sm" :class="dark ? 'text-slate-500' : 'text-slate-400'">Aucun enregistrement.</td>
                        </tr>
                    </tbody>
                </table>
                </div>
                <Pagination :links="departements.links" :meta="departements" />
            </div>
        </div>
    </AuthenticatedLayout>
</template>
