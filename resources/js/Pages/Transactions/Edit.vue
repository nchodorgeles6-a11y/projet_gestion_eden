...
const { dark } = useTheme();
const props = defineProps({ transaction: Object, users: Array, departements: Array });
const form = useForm({
    user_id: props.transaction.user_id,
    departement_id: props.transaction.departement_id,
    type: props.transaction.type,
    montant: props.transaction.montant,
    description: props.transaction.description,
});
const submit = () => form.put(`/transactions/${props.transaction.id}`);
    ...
    <AuthenticatedLayout>
        <div class="p-4 sm:p-6 max-w-2xl">
            <div class="flex items-center justify-between mb-5">
                <div>
                    <h2 class="text-base font-extrabold" :class="dark ? 'text-white' : 'text-slate-800'">Modifier la transaction</h2>
                    <p class="text-[11px] mt-0.5" :class="dark ? 'text-slate-400' : 'text-slate-500'">Modifiez les informations ci-dessous</p>
                </div>
                <Link href="/transactions" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold border transition-all"
                ...
                <form @submit.prevent="submit" class="space-y-4">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select v-model="form.departement_id" required class="w-full rounded-xl border px-3 py-2.5 text-sm transition-all focus:outline-none focus:ring-2" :class="inputCls(dark)">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select v-model="form.type" required class="w-full rounded-xl border px-3 py-2.5 text-sm transition-all focus:outline-none focus:ring-2" :class="inputCls(dark)">
                        ...
                        <div>
                            <label class="block text-xs font-semibold mb-1.5" :class="dark ? 'text-slate-300' : 'text-slate-700'">Montant (FCFA)</label>
                            <input v-model="form.montant" type="number" required placeholder="Ex: 500000"
                    ...
                    <div>
                        ...
                        <textarea v-model="form.description" rows="3" placeholder="Description de la transaction"
                            class="w-full rounded-xl border px-3 py-2.5 text-sm transition-all focus:outline-none focus:ring-2 resize-none" :class="inputCls(dark)"></textarea>
                    </div>
                    <div class="pt-2">
