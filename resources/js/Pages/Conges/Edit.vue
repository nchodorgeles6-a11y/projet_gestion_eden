...
const { dark } = useTheme();
const props = defineProps({ conge: Object, users: Array, motifs: Array });
const form = useForm({
    user_id: props.conge.user_id,
    motif_id: props.conge.motif_id,
    date_debut: props.conge.date_debut,
    date_fin: props.conge.date_fin,
});
...
<template>
    <Head title="Modifier Congé" />
    <AuthenticatedLayout>
        <div class="p-4 sm:p-6 max-w-2xl">
            <div class="flex items-center justify-between mb-5">
                <div>
                    <h2 class="text-base font-extrabold" :class="dark ? 'text-white' : 'text-slate-800'">Modifier le congé</h2>
