...
const destroy = (id) => {
    if (confirm('Voulez-vous supprimer ce rôle ?')) router.delete(`/roles/${id}`);
};
</script>
    ...
    <AuthenticatedLayout>
        <div class="p-4 sm:p-6">
            ...
            <div class="flex items-center justify-between mb-5">
                <div>
                    <h2 class="text-base font-extrabold" :class="dark ? 'text-white' : 'text-slate-800'">Rôles</h2>
                    <p class="text-[11px] mt-0.5" :class="dark ? 'text-slate-400' : 'text-slate-500'">
                        {{ roles?.meta?.total ?? 0 }} enregistrement(s)
                    </p>
                </div>
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody>
                        <tr v-for="(role, i) in roles.data" :key="role.id"
                            ...
                            <td class="px-4 py-2.5 text-[12px] font-medium" :class="dark ? 'text-slate-200' : 'text-slate-700'">{{ role.nom }}</td>
