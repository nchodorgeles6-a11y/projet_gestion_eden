...
const { dark } = useTheme();
const props = defineProps({ poste: Object });
const form = useForm({
    nom: props.poste.nom,
    description: props.poste.description,
    departement_id: props.poste.departement_id,
});
const submit = () => form.put(`/postes/${props.poste.id}`);
...
<template>
    <Head title="Modifier Poste" />
    <AuthenticatedLayout>
        <div class="p-4 sm:p-6 max-w-2xl">
            <div class="flex items-center justify-between mb-5">
                <div>
                    <h2 class="text-base font-extrabold" :class="dark ? 'text-white' : 'text-slate-800'">Modifier le poste</h2>
                    <p class="text-[11px] mt-0.5" :class="dark ? 'text-slate-400' : 'text-slate-500'">Modifiez les informations ci-dessous</p>
                </div>
                <Link href="/postes" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold border transition-all"
