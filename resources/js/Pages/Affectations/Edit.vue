...
const { dark } = useTheme();
const props = defineProps({ affectation: Object, users: Array, postes: Array });

const form = useForm({
    date_debut:       props.affectation.date_debut,
    date_fin:         props.affectation.date_fin,
    user_id:          props.affectation.user_id,
    poste_id:         props.affectation.poste_id,
    motif_changement: props.affectation.motif_changement ?? '',
});

const submit = () => form.put(`/affectations/${props.affectation.id}`);
...
// Pré-sélectionner le département du poste actuel
const currentPosteDept = props.postes.find(p => p.id === props.affectation.poste_id)?.departement?.id ?? '';
const selectedDept = ref(currentPosteDept);
...
<template>
    <Head title="Modifier Affectation" />
    <AuthenticatedLayout>
        <div class="p-4 sm:p-6">
            ...
            <div class="flex flex-wrap items-start justify-between gap-3 mb-6">
                <div>
                    ...
                    <p class="text-[11px] mt-0.5" :class="sL(dark)">Modifiez les informations ci-dessous</p>
                </div>
            ...
            <div class="grid grid-cols-1 xl:grid-cols-3 gap-5 items-start">
                ...
                <form @submit.prevent="submit"
                    ...
                    <!-- Salaire capturé (readonly) -->
                    <div v-if="affectation.salaire_capture"
                        class="rounded-xl border px-4 py-3"
                        :class="dark ? 'bg-[#0d1117] border-[#30363d]' : 'bg-slate-50 border-slate-200'">
                        <p class="text-[10px] font-bold uppercase tracking-wider mb-1" :class="sL(dark)">
                            Salaire capturé lors de cette affectation
                        </p>
                        <p class="text-sm font-extrabold text-[#760078]">{{ fmt(affectation.salaire_capture) }}</p>
                    </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="rounded-2xl border p-4 transition-colors"
                        :class="dark ? 'bg-[#161b22] border-[#21262d]' : 'bg-white border-slate-200 shadow-sm'">
                        <p class="text-[10px] font-bold uppercase tracking-widest mb-3" :class="sL(dark)">Employé</p>
                    ...
                    <div class="rounded-2xl border p-4 transition-colors"
                        :class="dark ? 'bg-[#161b22] border-[#21262d]' : 'bg-white border-slate-200 shadow-sm'">
                        <p class="text-[10px] font-bold uppercase tracking-widest mb-3" :class="sL(dark)">Poste</p>

                        <template v-if="selectedPoste">
