...
defineProps({ postes: Object });

const destroy = (id) => {
    if (confirm('Voulez-vous supprimer ce poste ?')) router.delete(`/postes/${id}`);
};
</script>
    ...
    <AuthenticatedLayout>
        <div class="p-4 sm:p-6">
            ...
            <div class="flex items-center justify-between mb-5">
                <div>
                    <h2 class="text-base font-extrabold" :class="dark ? 'text-white' : 'text-slate-800'">Postes</h2>
                    <p class="text-[11px] mt-0.5" :class="dark ? 'text-slate-400' : 'text-slate-500'">
                        {{ postes?.meta?.total ?? 0 }} enregistrement(s)
                    </p>
                </div>
                <Link href="/postes/create"
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr :class="dark ? 'bg-[#0d1117]' : 'bg-slate-50'">
                            <th class="px-4 py-2.5 text-left text-[10px] font-bold tracking-wider uppercase" :class="dark ? 'text-slate-500' : 'text-slate-400'">Nom</th>
                            <th class="px-4 py-2.5 text-left text-[10px] font-bold tracking-wider uppercase" :class="dark ? 'text-slate-500' : 'text-slate-400'">Description</th>
                    ...
                    <tbody>
                        <tr v-for="(poste, i) in postes.data" :key="poste.id"
                            ...
                            <td class="px-4 py-2.5 text-[12px] font-medium" :class="dark ? 'text-slate-200' : 'text-slate-700'">{{ poste.nom }}</td>
                            <td class="px-4 py-2.5 text-[12px]" :class="dark ? 'text-slate-400' : 'text-slate-500'">{{ poste.description }}</td>
                            <td class="px-4 py-2.5 text-right">
                                <div class="inline-flex items-center gap-2">
                                    <Link :href="`/postes/${poste.id}/edit`"
                        ...
                        <tr v-if="!postes?.data?.length">
                            <td colspan="3" class="px-4 py-8 text-center text-sm" :class="dark ? 'text-slate-500' : 'text-slate-400'">Aucun enregistrement.</td>
                        </tr>
                    </tbody>
                </table>
                <Pagination :links="postes.links" :meta="postes" />
            </div>
        </div>
    </AuthenticatedLayout>
</template>
